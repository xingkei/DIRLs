...
import itertools
from utils.fig import plot_fig, hunxiao, ROCs
...
def our_args():
    ...
    parser.add_argument('--alpha', type=float, default=1, help='蒸馏损失loss2')
    parser.add_argument('--beta', type=float, default=0.0004, help='exp损失权重loss3')
    parser.add_argument('--lam', type=float, default=1, help='coral损失权重loss4')
...
class DIRLs(nn.Module):
    ...
    def teanettrain(self, dataloaders):
        teaModel = self.teaNet.to(device)
        optimizer = optim.Adam(teaModel.parameters(), lr=self.args.lr)
        teaModel.train()
        ...
        for epoch in range(epochs):    # 迭代epoch
            ...
            for batch in range(batchnum):    # 迭代batch
                ...
                with torch.no_grad():
                    b, a = signal.butter(N=2, Wn=[2300, 5300], btype='band', fs=12800)
                    x_filtered = signal.filtfilt(b, a, all_x.cpu().numpy())
                    hilbert_x = signal.hilbert(x_filtered)
                    all_x1 = torch.angle(torch.fft.fftn(torch.tensor(hilbert_x))).to(device)
                    tfea = self.teab(self.teaf(torch.unsqueeze(all_x1.float(), dim=1))).detach()
...
class TestDIRLs:
    ...
    def acc(self, args):
        ...
        my_array6 = np.array(labels.cpu().tolist())
        # 使用scipy的savemat函数保存为.mat文件
        io.savemat('./output/con_pad.mat', {'y_pred': my_array3, 'y_true': my_array4, 'rep': my_array5, 'lab': my_array6})
...
if __name__ == "__main__":
    ...
    args = our_args()
    # 创建数据加载器
    dataset1 = create_dataset('.\Labbearing/condition1.mat', Normalize=args.normalization)
    trainloader1 = DataLoader(dataset1, batch_size=args.batchSize, shuffle=args.shuffle, drop_last=True)
    dataset2 = create_dataset('.\Labbearing/condition2.mat', Normalize=args.normalization)
    trainloader2 = DataLoader(dataset2, batch_size=args.batchSize, shuffle=args.shuffle, drop_last=True)
    dataset3 = create_dataset('.\Labbearing/condition3.mat', Normalize=args.normalization)
    trainloader3 = DataLoader(dataset3, batch_size=args.batchSize, shuffle=args.shuffle, drop_last=True)
    dataset4 = create_dataset('.\Labbearing/condition4.mat', Normalize=args.normalization)

    trainset = torch.utils.data.ConcatDataset([dataset1, dataset2, dataset3])
    trainlaoder = DataLoader(trainset, batch_size=args.batchSize, shuffle=True, drop_last=True)

    testloader = DataLoader(dataset3, batch_size=2000, shuffle=True, drop_last=True)
    # 初始化模型
    model = ConvNet()
    model1 = DIRLs(args)
    # 融合多个dataset
    ...
    MyDirls.teanettrain(dataloaders=minibatch[0])
    print('教师模型训练完毕...\n')
